...
# list private notice
def list_private(params: dict, receiver: str):
    ...
    if keyword:
        queryset = queryset.filter(Q(title__contains=keyword) | Q(creator__contains=keyword))
...
def privates(request: HttpRequest):
    if not request.user.is_authenticated:
        return AuthFailed()
    param = request.GET
    return list_private(param, str(request.user.pk))
